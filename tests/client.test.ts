...
describe('TapsaSMS', () => {
  ...
  it('rejects a missing API key', () => {
    ...
    expect(() => new TapsaSMS({ apiKey: '' })).toThrow('apiKey is required');
  });

  it('sends one SMS with authentication and JSON', async () => {
  ...
  it('supports multiple recipients and base URL overrides', async () => {
    ...
    globalThis.fetch = fetchMock;
    await new TapsaSMS({ apiKey: 'secret', baseUrl: 'http://localhost:3000/' }).sendSMS({ phoneNumbers: ['255712345678', '255713456789', '255714567890'], message: 'Hello everyone' });
  ...
  for (const status of [400, 401, 402, 403, 429, 500]) {
    it(`preserves HTTP ${status} API errors`, async () => {
      ...
      await expect(new TapsaSMS({ apiKey: 'secret' }).getBalance()).rejects.toSatisfy((error: unknown) => {
  ...
  it('reports malformed responses', async () => {
    ...
    await expect(new TapsaSMS({ apiKey: 'secret' }).getBalance()).rejects.toMatchObject({ status: 200, code: 'MALFORMED_RESPONSE' });
  });
});
